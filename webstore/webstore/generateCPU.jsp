...
<%
	...
	if(load ==null){
		System.out.println(new java.util.Date()+"	No load");
	%>
    <span onclick="javascript:generateCpuLoad();	">Click to Generate High CPU   (continous while loop)</span>
		<%
	}
	else{
		%>
    <span onclick="javascript:generateCpuLoad();	">Click to Generate High CPU   (continous while loop)</span>
  <!--   <span onclick="javascript:generateCpuLoad();	">Hiking CPU </span> -->
		<%
		...
		try {
			...
			long cpuDuration = Long.parseLong(duration);
			System.out.println(new java.util.Date()+"	CPU loop duration given is "+cpuDuration);
