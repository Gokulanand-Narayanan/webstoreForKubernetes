...
//		productId = -1;
	...
	private void insertIntoCheckOutAddress() {
		...
		String cart_order_address = "SELECT id, quote_id, created_at, updated_at, customer_address_id, region_id, customer_id, fax, region, postcode, lastname, street, city, email, telephone, country_id, firstname, address_type, prefix, middlename, suffix, company FROM checkout_quote_address where sleep(0.1)";
		try {
	...
	private void getCustomerDetails() {
		...
		String customerDetailsQuery = "SELECT id, customer_group, prefix, first_name, middle_name, last_name, email, date_of_birth, tax_number, gender, password, company, street_address1, street_address2, street_address3, city, country, state, postal_code, phone, fax FROM customer_master where sleep(3.1)"; 
		try {
			Statement statement = connection.createStatement();
