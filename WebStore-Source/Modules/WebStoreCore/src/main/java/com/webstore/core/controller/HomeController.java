 ...
 * #L%
 */
package com.webstore.core.controller;
...
import com.webstore.core.jms.MessageSender;
import com.webstore.core.model.ProductsMaster;
import com.webstore.core.uricostants.ServerUris;
import com.webstore.core.uricostants.URIConstants;
...
public class HomeController {   	
	...
	public @ResponseBody String welcome() {
		return "Welcome to WebStore Core Engine";
	}
	
	
	@RequestMapping(value = "/login", method = RequestMethod.POST)
	public @ResponseBody String loginToBigStore(@RequestParam(required = false, value = "params") JSONObject jsonParams) {
	...
	public @ResponseBody String loginToJvoid(@RequestParam(required = false, value = "callback") String callback, @RequestParam(required = false, value = "params") JSONObject jsonParams) {
		...
		String url = ServerUris.CORE_SERVER_URI+"/oauth/token?grant_type=password&client_id="+clientid+"&client_secret="+clientsecret+"&username="+username+"&password="+password;
	...
	public @ResponseBody String jvoidLoginTester(@RequestParam("params") String jsonParams) {
		...
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(ServerUris.CORE_SERVER_URI+"/login")
	...
	@RequestMapping("/products-by-cat")
	...
	public @ResponseBody String orderProductNowById(HttpServletRequest request,@RequestParam("cartId") String cartId, @RequestParam("prodId") String productId) {
		RestTemplate restTemplate = new RestTemplate();
		HttpHeaders headers = new HttpHeaders();
		headers.set("Accept", MediaType.APPLICATION_JSON_VALUE);
	...
	public @ResponseBody String removeItemsFromCartById(HttpServletRequest request,@RequestParam("cartId") String cartId, @RequestParam("prodId") String prodId) {
		RestTemplate restTemplate = new RestTemplate();
		HttpHeaders headers = new HttpHeaders();
		headers.set("Accept", MediaType.APPLICATION_JSON_VALUE);
	...
	public @ResponseBody String jvoidCheckoutCart(HttpServletRequest request,@RequestParam("params") String jsonParams) {
		RestTemplate restTemplate = new RestTemplate();
		HttpHeaders headers = new HttpHeaders();
		headers.set("Accept", MediaType.APPLICATION_JSON_VALUE);
	System.out.println("HomeController.jvoidCheckoutCart()  ################################  ");
		...
		System.out.println("HomeController.jvoidCheckoutCart() returnString "+returnString );
		...
		System.out.println("HomeController.jvoidCheckoutCart() result "+result);
		
		String response = "200";
		if(result.equals("Success")){
			// add the json params into jms queue
				...
				System.out.println("HomeController.jvoidChrckoutCart() ***************** "+builder1.build().toUri() + " jsonParams  "+jsonParams);
				...
				System.out.println("HomeController.jvoidChrckoutCart() "+jsonParams);
				...
				String result1 = "";
				try {
					result1 = returnJsonObj1.getString("orderId");
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				...
				request.getSession().setAttribute("cart",result1);
				//System.out.println("------------GET SESSION -----------"+request.getSession().getAttribute("cart"));
			messageSender.sendMessage(jsonParams);
		}
		return response;
	}
}